  ...
  `,
    ...
    void mainImage( out vec4 fragColor, in vec2 fragCoord )
    {
      vec2 uv = (fragCoord.xy-.5*u_resolution.xy)/u_resolution.y;
        ...
        f *= .9;
vec3 bgColor = vec3(0.0, 0.5, 1.0); // blue background
vec3 waveColor = vec3(1.0);         // white waves
...
<Canvas className={className} frameloop="always">
  <ShaderPlane
    vertexShader={vertexShader}
    fragmentShader={fragmentShader}
...
export default function InfiniteHero() {
	...
	return (
		<div
			ref={rootRef}
			className="relative h-svh w-full overflow-hidden bg-white text-white"
		>
...
<Navbar/>
  ...
  <div className="text-center">
    ...
    <h1
      ref={h1Ref}
      className="mx-auto max-w-4xl lg:max-w-5xl text-[clamp(2.25rem,6vw,4rem)]  leading-[0.95] tracking-tight font-semibold"
    >
      بایو وِیو;  موج سلامت هوشمند شما
    </h1>

    {/* Subheading / description */}
    <p
      ref={pRef}
      className="mx-auto mt-4 max-w-4xl md:text-balance text-lg md:text-lg font-light tracking-tight text-white/80"
    >
      تجربه‌ای نوین در بررسی سلامت بدن با استفاده از فناوری بیورزونانس
      بایو ویو به شما امکان می‌دهد وضعیت جسمی و انرژی بدن خود را به دقت تحلیل کنید
      و راهکارهای بهبود سلامت را دریافت نمایید.
    </p>
    ...
    >
      <button
        type="button"
        className="group flex relative overflow-hidden border border-white/30 bg-gradient-to-r from-white/20 to-white/10 px-6 py-3 text-lg rounded-lg font-medium tracking-wide text-white backdrop-blur-sm transition-[border-color,background-color,box-shadow] duration-500 hover:border-white/50 hover:bg-white/20 hover:shadow-lg hover:shadow-white/10 cursor-pointer"
      >		

        شروع بررسی سلامتی 
		<ArrowLeft className="ml-2 mt-1"/>
      </button>

      <button
        type="button"
        className="group relative px-6 py-3 text-md font-medium tracking-wide text-white/90 transition-[filter,color] duration-500 hover:drop-shadow-[0_0_6px_rgba(255,255,255,0.6)] hover:text-white cursor-pointer"
      >
        مشاهده خدمات بیشتر
      </button>
    </div>
...
<div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20">
  <div className="flex flex-col items-center cursor-pointer animate-scroll-wiggle">
    <svg
    ...
    <span className="mt-2 text-md text-white/70">اسکرول کنید</span>
  </div>
</div>
